...
* `python -m pytest tests -q` - 50 passed. The suite builds real SCP flux from
  a real PC floppy image and decodes it back.
...
* **Multi-core decode.** A three-capture 40-cylinder run takes about 30
  seconds, all of it in the pure-Python PLL. The tracks are independent, so
  `concurrent.futures.ProcessPoolExecutor` over `(cyl, head)` would cut it by
  the core count. Skipped for v1 because it complicates the progress bar and
  the error paths for a run that is already shorter than the disk read.
...
* **Weak-bit detection.** A sector that never agrees with itself across many
  revolutions of the same capture is probably protection, not damage, and
  diskstack currently reports it as unresolved damage.
